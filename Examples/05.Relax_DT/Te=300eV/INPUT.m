...
Te_Ti1=2;
Te_Ti2=3;
vTe=sqrt(0.3/511);% 300eV
vTi1=vTe*sqrt(1/Te_Ti1/mass_ion1/1836);
vTi2=vTe*sqrt(1/Te_Ti2/mass_ion2/1836);
% species:
dimsp=3;
Z=[-1,1,1];
mass=[1,1836*2,1836*3];
number_ratio=[1,0.5,0.5]; %总粒子数之比
% control:
Nx=4;
xmax=1;
Nv=[54,108,108];
vmax=[7.0*vTe,7.0*vTi1,7.0*vTi2];
Nt=300000;
dt=0.2;
lmax=[0,0,0];
mmax=[0,0,0];
...
file_start_number=200000;%-1：从分布函数算起；非负：从编号file_start_number的文件算起。
% interruption
...
ne=1e+23;   % 单位cm^-3，n=1时的实际密度。 
Te=300;    % eV 影响ei碰撞截断，对结果基本无影响，大致给对即可。
Ivc=[3,3,3];
% conservation
...
bdy.Ft='outflow';%periodic;outflow;laser;
% IB heating 不要使用
flag.IB_heating=0;
