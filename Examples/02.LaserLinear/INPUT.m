...
flag.init_pause=1;
flag.debug_info=1;
file_write=1;%写文件间隔，一个dt为一个间隔。
file_start_number=6900;%-1：从分布函数算起；非负：从编号file_start_number的文件算起。
% interruption
number_tol=0;%当电子总数变化率超过此值时结束运行,略大于零，等于0时无效。
max_multiple=100;%当电子分布函数的最大值超过原先的此值的倍数时结束运行，等于0时无效。
% distributions:
% 分布函数：函数名，其他参数（热速度等）。
% 密度轮廓：轮廓类型，参数。
